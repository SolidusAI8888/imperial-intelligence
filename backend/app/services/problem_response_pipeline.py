...
def _selected_profile_row(problem_id: str, person_id: str) -> dict:
    ...
    if len(matches) != 1:
        raise ValueError(
            f"Problem {problem_id} must contain exactly one candidate row for {person_id}"
        )
    return matches[0]
...
def build_selected_runtime_context(problem_id: str, question: str | None = None) -> RuntimeContext:
    ...
    experiences = [
        heu for heu in load_person_experiences(person_id) if heu.heu_id in set(plan.heu_ids)
    ]
    ...
    insights = [
        ...
        if insight.insight_id in set(plan.insight_ids)
    ]
    role_links = [
        link for link in load_person_role_links(person_id) if link.heu_id in set(plan.heu_ids)
    ]
...
def build_grounded_response_bundle(
    problem_id: str,
    question: str | None = None,
) -> GroundedResponseBundle:
    ...
    if evidence_ids != plan.evidence_ids:
        raise ValueError(
            f"Problem {problem_id} selected runtime evidence diverges from candidate profile"
        )
